...
from typing import cast, Any, Union, Optional, List, Tuple
...
def apply_bitflip_bias(
    ...
    for site, prob_bias in zip(cast(List[int], sites_to_bias), prob_bias_list):
...
def linear_code_prepare_message(
    ...
    error_model: Any = BinarySymmetricChannel,
...
def decode_custom(
    ...
    assert isinstance(error_mps, CanonicalMPS)
    if sites_to_bias:
        if bias_type == "Bitflip":
            if not silent:
