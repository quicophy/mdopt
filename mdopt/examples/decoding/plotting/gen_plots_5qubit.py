"""Generate publication-ready PDFs for the 5-qubit perfect code."""
...
num_qubits = 5
num_logicals = 2
num_sites = 2 * num_qubits + num_logicals  # 12
...
bond_dim_vals = [np.inf, 128, 64, 32, 16, 8, 4, 2]
...
for chi in tqdm(bond_dim_vals):
    chi_use = 10000 if np.isinf(chi) else int(chi)
    mps = create_custom_product_state(string=logicals_state + error_state)
    mps = apply_bitflip_bias(mps=mps, prob_bias_list=0.01, sites_to_bias=sites_to_bias)
    for i in [0, 1]:
...
def decode_all(paulis):
    return [
        ...
        for error in tqdm(paulis)
    ]


one_q = decode_all(one_qubit_paulis)
two_q = decode_all(two_qubit_paulis)
three_q = decode_all(three_qubit_paulis)

labels_order = ["I", "X", "Z", "Y"]
datasets = [
    (one_q, "1-qubit errors"),
    (two_q, "2-qubit errors"),
    (three_q, "3-qubit errors"),
]

fig, axes = plt.subplots(1, 3, figsize=(7, 2.8), constrained_layout=True)
for ax, (dist, title) in zip(axes, datasets):
    counts = Counter(map_distribution_to_pauli(dist))
