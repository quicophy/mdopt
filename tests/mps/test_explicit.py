...
def test_explicit_marginal():
    ...
    mps_prod = create_simple_product_state(
        num_sites=num_sites, which="0", phys_dim=phys_dim, form="Explicit"
    )
    ...
    mps_prod = create_simple_product_state(
        num_sites=num_sites, which="+", phys_dim=phys_dim, form="Explicit"
    )
    ...
    mps_marginalised = mps_prod.marginal(sites_to_marginalise)
    for sing_values_0, sing_values_1 in zip(
        mps_prod.singular_values, mps_prod_result.singular_values
    ):
        assert np.isclose(sing_values_0, sing_values_1).all()
    ...
    for _ in range(10):
        ...
        if isinstance(mps_marginalised, CanonicalMPS):
            assert mps_marginalised_canonical.num_sites == len(sites_left)
