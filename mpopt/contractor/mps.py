"""
    This module contains the MPS class and some other tools.
    Written by Alex Berezutskii inspired by TenPy in 2020-2021.
"""

from functools import reduce
import numpy as np
from mpopt.common_tensors import Z
...
def dephase_tensor(tensor):
    ...
    """
    z_tensor_z = np.einsum("ab, iacl, cd -> ibdl", Z, tensor, Z)
...
class MPS:
    ...
    def single_site_wavefunction(self, site):
        """
        Computes the single-site wave function at a given site.
    ...
    def two_sites_wavefunction(self, site):
        ...
        return np.tensordot(
            self.single_site_wavefunction(site), self.tensors[next_site], [2, 0]
        )  # vL i [vR], [vL] j vR
